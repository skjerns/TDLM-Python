...
def signflit_test(sx, n_perms=1000, rng=None):
    ...
    Aggregates per-column t-statistics into a one-sided (positive) max-t. The null
    is generated by flipping row signs; standard errors are computed once from the
    observed data and reused across permutations.

    Parameters
    ----------
    sx : array_like, shape (n_obs, n_cols)
        Data matrix. NaNs allowed.
    ...
    SignflipResult
        Namedtuple with:
        pvalue : float
            Finite-sample–corrected familywise p-value for the positive max-t.
        t_base : float
            Observed max t-statistic across columns.
        t_perm : ndarray of shape (n_perms,)
            Max t-statistic per permutation (one-sided).
        thresholds : ndarray of shape (n_cols,)
            Nonnegative increase in each column mean required to reach the 95th
            percentile familywise threshold, computed as
            maximum(0, t_crit * se - mean), where t_crit is the 95th percentile of
            the permutation max-t distribution and se is the columnwise standard
            error from the observed data.
    """
    if sx.ndim != 2:
    ...
    # Get original mean values that would have been necessary to reach 95%
    # TODO: Check if this is valid, I'm pretty sure that it is not
    thresholds = np.percentile(np.abs(t_perm), 95, axis=0)*se - mean_seq
    # raise Exception('thresholds are wrong, fix me')
    result = namedtuple('SignflipResult', ['pvalue', 't_obs', 't_perms', 'thresholds'])
    return result(pvalue, t_obs, t_perms, thresholds)
